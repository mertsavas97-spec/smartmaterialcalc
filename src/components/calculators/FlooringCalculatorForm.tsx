"use client";

import { useState } from "react";
import { CalculatorFormActions } from "./CalculatorFormActions";
import { CalculatorResultCard } from "./CalculatorResultCard";
import { CalculatorTipBox } from "./CalculatorTipBox";
import { CalculatorValidationAlert } from "./CalculatorValidationAlert";
import { CalculatorResultsEmpty } from "./CalculatorResultsEmpty";
import {
  CALCULATOR_INPUT_CLASSNAME,
  parseNumber,
} from "@/lib/calculator-utils";
import { trackCalculatorSubmit } from "@/lib/analytics";
import { validateFlooringInputs } from "@/lib/calculator-form-utils";
import {
  calculateFlooring,
  DEFAULT_FLOORING_INPUTS,
  type FlooringCalculatorInputs,
} from "@/lib/flooring-calculator";

export function FlooringCalculatorForm() {
  const [inputs, setInputs] = useState<FlooringCalculatorInputs>(
    DEFAULT_FLOORING_INPUTS
  );
  const [results, setResults] = useState<ReturnType<
    typeof calculateFlooring
  > | null>(null);
  const [hasCalculated, setHasCalculated] = useState(false);
  const [validationError, setValidationError] = useState<string | null>(null);

  function handleCalculate(e: React.FormEvent) {
    e.preventDefault();
    const error = validateFlooringInputs(inputs);
    if (error) {
      setValidationError(error);
      setHasCalculated(false);
      setResults(null);
      return;
    }
    setValidationError(null);
    setResults(calculateFlooring(inputs));
    setHasCalculated(true);
    trackCalculatorSubmit("flooring-calculator");
  }

  function handleReset() {
    setInputs(DEFAULT_FLOORING_INPUTS);
    setResults(null);
    setHasCalculated(false);
    setValidationError(null);
  }

  function updateField<K extends keyof FlooringCalculatorInputs>(
    field: K,
    value: string
  ) {
    setInputs((prev) => ({
      ...prev,
      [field]: parseNumber(value, prev[field]),
    }));
  }

  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
      <div className="rounded-[var(--radius-lg)] border border-card-border bg-white p-6 shadow-sm">
        <h2 className="text-lg font-semibold text-text-primary">
          Enter room dimensions
        </h2>
        <form
          className="mt-5 space-y-4"
          aria-label="Flooring calculator inputs"
          onSubmit={handleCalculate}
        >
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label htmlFor="flooring-length" className="block text-sm font-medium text-text-primary">
                Room length (ft)
              </label>
              <input
                id="flooring-length"
                type="number"
                min="0"
                step="0.1"
                value={inputs.roomLength}
                onChange={(e) => updateField("roomLength", e.target.value)}
                className={CALCULATOR_INPUT_CLASSNAME}
              />
            </div>
            <div>
              <label htmlFor="flooring-width" className="block text-sm font-medium text-text-primary">
                Room width (ft)
              </label>
              <input
                id="flooring-width"
                type="number"
                min="0"
                step="0.1"
                value={inputs.roomWidth}
                onChange={(e) => updateField("roomWidth", e.target.value)}
                className={CALCULATOR_INPUT_CLASSNAME}
              />
            </div>
          </div>
          <div>
            <label htmlFor="flooring-waste" className="block text-sm font-medium text-text-primary">
              Waste allowance (%)
            </label>
            <input
              id="flooring-waste"
              type="number"
              inputMode="numeric"
              min="0"
              step="1"
              value={inputs.wastePercent}
              onChange={(e) => updateField("wastePercent", e.target.value)}
              className={CALCULATOR_INPUT_CLASSNAME}
            />
          </div>
          <div>
            <label htmlFor="flooring-box-coverage" className="block text-sm font-medium text-text-primary">
              Box coverage (sq ft)
            </label>
            <input
              id="flooring-box-coverage"
              type="number"
              inputMode="decimal"
              min="0"
              step="0.1"
              value={inputs.boxCoverageSqFt}
              onChange={(e) => updateField("boxCoverageSqFt", e.target.value)}
              className={CALCULATOR_INPUT_CLASSNAME}
            />
          </div>
          <CalculatorValidationAlert message={validationError} />
          <CalculatorFormActions onReset={handleReset} />
        </form>
      </div>

      <div className="rounded-[var(--radius-lg)] border border-card-border bg-white p-6 shadow-sm">
        <h2 className="text-lg font-semibold text-text-primary">
          Your estimate
        </h2>
        {hasCalculated && results ? (
          <div className="mt-5 space-y-4">
            <CalculatorResultCard
              label="Recommended boxes"
              value={`${results.recommendedBoxes} boxes`}
              highlight
              subtext="Rounded up to whole boxes"
            />
            <CalculatorResultCard
              label="Boxes needed"
              value={`${results.boxesNeeded.toFixed(1)} boxes`}
            />
            <CalculatorResultCard
              label="Floor area"
              value={`${Math.round(results.floorAreaSqFt)} sq ft`}
            />
            <CalculatorResultCard
              label="Area with waste"
              value={`${Math.round(results.areaWithWasteSqFt)} sq ft`}
            />
            <div className="rounded-[var(--radius-sm)] border border-card-border px-4 py-3 text-xs text-text-muted">
              <p>Waste assumption: {inputs.wastePercent}%</p>
              <p className="mt-1">
                Box coverage assumption: {inputs.boxCoverageSqFt} sq ft per box
              </p>
            </div>
            <CalculatorTipBox>
              <strong className="text-text-primary">Tip:</strong> Add 10% for
              straight installs and 12–15% for diagonal layouts. Keep one extra
              box for future repairs — styles get discontinued.
            </CalculatorTipBox>
          </div>
        ) : (
          <CalculatorResultsEmpty />
        )}
      </div>
    </div>
  );
}
